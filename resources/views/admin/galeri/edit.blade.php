...
@section('content')
  <div class="max-w-2xl mx-auto px-6 py-10">
    ...
    <form action="{{ route('admin.galeri.update', $galeri) }}" method="POST" enctype="multipart/form-data" class="space-y-5 bg-white p-6 rounded-lg border border-slate-200">
      @csrf
      @method('PUT')
      ...
      <div>
        ...
        <textarea name="deskripsi" rows="4"
                  class="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900">{{ old('deskripsi', $galeri->deskripsi) }}</textarea>
      ...
      <div>
        ...
        @if ($galeri->gambar)
          <img src="{{ Storage::url($galeri->gambar) }}" class="w-32 h-20 object-cover rounded-md mb-2">
        @endif
      ...
      <div class="flex gap-3">
        ...
        <a href="{{ route('admin.galeri.index') }}" class="px-5 py-2.5 rounded-md text-sm border border-slate-300 hover:bg-slate-50">Batal</a>
      </div>
    </form>
  </div>
@endsection
